...
export default function CustomerMessagerie({ params }: { params: { bookingId?: string } }) {
    ...
    const loadConversations = async () => {
      try {
        ...
        if (userConversations.length > 0) {
          // If a specific bookingId was passed, try to select that conversation
          if (bookingId) {
            const specificConversation = userConversations.find(conv => conv.booking_id === bookingId);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      try {
        const conversationMessages = await getConversationMessages(user.id, selectedConversation.booking_id);
        setMessages(conversationMessages);
        
        // Mark messages as read when viewing the conversation
        if (selectedConversation.other_user.id) {
          await markMessagesAsRead(user.id, selectedConversation.other_user.id, selectedConversation.booking_id);
      ...
      setConversations(prev => prev.map(conv =>
        conv.id === selectedConversation.id
          ? {
              ...
              // Update the unread count for the other party (though it would be 0 for the sender)
            }
          : conv
      ));
    } catch (error) {
